<template>
	...
	<view class="storedels">
		...
		<view class="storetop">
			<swiper class='u-wrp-bnr' indicator-dots='true' autoplay='true' interval='5000' duration='1000' circular='true'>
				<block class="bannerblock" v-for="(item,index) in bannerList" :key="index">
					<swiper-item>
						<image class="storetopimg u-img-slide" :src="'https://xcx.zhongshengzb.com:8089/shoppingImg/images/'+item.amUrl" mode=""></image>
					</swiper-item> 
				</block>
			</swiper>
		...
		<view class="storetable">
				...
				<view class="tab-con">
					<view class="tab-connone" :class="{tabconshow:tableIndex==1}">
						<image class="u-img-slide image" :src="'https://xcx.zhongshengzb.com:8089/shoppingImg/images/' + delsListimg" mode=""></image>
					</view>
						...
						<view class="tab-haopinglist tab-connone" >
							<view class="tab-haopinglist-whole" v-for="evaluationAllItem in evaluationAll" :key="evaluationAllItem.comId">
								<view class="tab-haopinglist-whole-view1">
									...
									<view class="view2"><text v-for="item in evaluationAllItem.remark">*</text></view>
		...
		<view class="buy">
			<van-goods-action>
			  <van-goods-action-icon icon="chat-o" text="客服" @click="giveService"/>
			  <van-goods-action-icon icon="cart-o" text="购物车" :info="buycartnumber" @click="gotToshoppingCart" />
			  <van-goods-action-icon icon="star-o" text="关注"  @click="Attention()"/>
	...
	<van-action-sheet :show="isshow1"
	   ...
	   <view class="isguige">
			...
			<view class="smallnumber">
				...
				<view class="smallnumber2"><van-stepper :value="buynumber" @change="setbuynumber" integer max="100" /></view>
			</view>
			<view class="queding">
	...
	<van-action-sheet :show="isshow2"
	   ...
	   <view class="isguige">
			...
			<view class="smallnumber">
				<view class="smallnumber1">数量</view>
				<view class="smallnumber2"><van-stepper :value="isbuynumber" @change="setbuynumber2" integer  max="100"/></view>
			</view>
			<view class="queding">
...
<script>
	...
	export default {
		...
		methods: {
			...
			JoinCart2(userId,orderStatus,sumPrice,payStat,storeId,commJson,body){ 
				const _this = this; //buynumber
				this.sumPrice = this.isbuynumber*this.delsList.commdityPrice  //商品价格
				console.log("商品价格是",this.sumPrice)
				_this.isshow2 = false
				uni.navigateTo({
					url:'../yesOrder/yesOrder?userId='+ _this.userid+"&orderStatus="+ 0 +"&sumPrice="+ _this.sumPrice +"&payStat="+ 0 +"&storeId="+_this.storeId+"&commdityCount="+ _this.commdityCount + "&commdityId=" +_this.commdityId + "&body=" + _this.body 
				})
			},
			// 切换
			table(e = 1) {
